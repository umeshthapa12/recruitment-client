...
export class BasicComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @ViewChild('renderAboutMe') aboutMeEl: ElementRef<HTMLElement>;

    alertModel: SnackbarModel = { type: AlertType.success, message: 'Success', title: 'Success' };
    ...
    private initProfile(_: ResponseModel) {
        ...
        // now we are gonna needed view child to be render so we can use html element
        this.cdr.detectChanges();

        if (!(p && p.aboutMe)) return;

        const about = JSON.parse(p.aboutMe);
        const ql = new Quill(this.aboutMeEl.nativeElement, {
            readOnly: true,
            modules: {
                toolbar: false
            }
        });
        ql.setContents(about);
    }
    ...
    onEdit(focusTo: string) {
        ...
        const instance = this.dialog.open(BasicFormComponent, {
            width: '850px',
            data: data ? data : null,
            autoFocus: false,
        });
    ...
    onDeleted(uidFilename: string) {
        ...
        this.bService.deleteProfilePicture(el.deleteUrl).pipe(
            ...
            if (el) {
                ...
                const fromLocal = localStorage.getItem('js-avatar');

                if (fromLocal && fromLocal.search(uidFilename)) {
    ...
    onSetProfilePicture(uidFilename: string) {
        ...
        const el = this.pictures.find(_ => _.fileName === uidFilename);
        this.bService.updateImgStatus('avatar', el.fileName)
            .pipe(takeUntil(this.toDestroy$), delay(this.wait))
            .subscribe({
                next: res => {
                    this.store.dispatch(new IsUserLoggedInAction(UserType.JobSeeker));
